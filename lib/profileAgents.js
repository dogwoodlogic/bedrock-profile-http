...
import {profileAgents, profileMeters} from '@bedrock/profile';
...
bedrock.events.on('bedrock-express.configure.routes', app => {
  ...
  app.post(
    ...
    asyncHandler(async (req, res) => {
      ...
      const {id: meterId} = await createMeter({
  ...
  app.post(
    ...
    asyncHandler(async (req, res) => {
      ...
      const now = Date.now();
      const ttl = bedrock.config['profile-http'].zcap.ttl;
  ...
  app.post(
    ...
    asyncHandler(async (req, res) => {
      ...
      // ensure profile agent `account` matches session account
      if (!secrets.token && profileAgent.account !== accountId) {
        throw new BedrockError(
          'The "account" is not authorized.',
          'NotAllowedError',
          {httpStatusCode: 403, public: true});
